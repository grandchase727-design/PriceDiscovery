...
| [directory_structure.md](directory_structure.md) | 파일 트리 + gitignore 규칙 (기존) | 코드베이스 신규 진입자 |
...
python3 fundamentals_pipeline.py          # yfinance fundamentals, ~5분
python3 finnhub_fundamentals.py           # Finnhub 보강, ~28분
...
1. `price_discovery.py` (or 해당 agent 파일) 수정
2. `python3 price_discovery.py` 재실행 → cache 갱신
3. 영향받는 docs/ 페이지 업데이트
4. dependency graph 재생성: `python3 draw_dependency_graph.py`
