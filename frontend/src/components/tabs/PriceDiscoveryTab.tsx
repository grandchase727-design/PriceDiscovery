...
function ClassificationCard({ def }: { def: ClsDef }) {
  const stageBadge =
    def.stage === "momentum" ? { label: "Momentum", cls: "bg-green-900/30 text-green-300 border-green-800/40" }
    : def.stage === "pre-momentum" ? { label: "Pre-Momentum", cls: "bg-cyan-900/30 text-cyan-300 border-cyan-800/40" }
    : { label: "Excluded", cls: "bg-red-900/30 text-red-300 border-red-800/40" };
  return (
    <div className="bg-[#0d1117] border border-gray-800 rounded-lg p-2.5 hover:border-gray-700 transition-colors">
      <div className="flex items-baseline justify-between gap-2 mb-1">
        <div className="flex items-baseline gap-1.5 min-w-0">
          <span className="text-base shrink-0">{def.emoji}</span>
          <span className="font-bold text-[12px] truncate" style={{ color: def.color }}>{def.name}</span>
        </div>
        <span className={`text-[8.5px] px-1.5 py-0.5 rounded border font-semibold shrink-0 ${stageBadge.cls}`}>
          {stageBadge.label}
        </span>
      </div>
      <div className="text-[12px] text-gray-200 font-medium mb-1">{def.ko}</div>
      <div className="text-[10px] text-gray-500 mb-1.5">
        <span className="text-gray-400 font-semibold">기준:</span> <span className="font-mono">{def.rule}</span>
      </div>
      <div className="text-[10.5px] text-gray-400 leading-snug">
        <span className="text-gray-300">의미:</span> {def.implication}
      </div>
    </div>
  );
}
...
function ClassificationDefinitionsPanel() {
  ...
  return (
    <div className="border border-gray-800 rounded-lg overflow-hidden">
      <button
        className="w-full px-4 py-2.5 text-left text-xs font-semibold bg-[#111827] hover:bg-[#1f2937] flex justify-between items-center"
        onClick={() => setOpen(!open)}
      >
        <span className="text-gray-400">
          Classification Definitions — Pre-Momentum / Momentum 분류 정의
          <span className="ml-2 text-[10px] text-gray-600">14 classifications · 4 그룹</span>
        </span>
      ...
      {open && (
        <div className="bg-[#0d1117] p-4 space-y-4">
          <div className="text-[11px] text-gray-500 leading-relaxed">
            ...
            <span className="text-gray-300"> 14개 classification</span> 중 하나로 분류됩니다.
            여기에 composite ≥ 55 + ADV ≥ $5M 조건을 적용해
            <span className="text-cyan-300"> Momentum</span> /
            <span className="text-cyan-300"> Pre-Momentum</span> /
            <span className="text-red-300"> Excluded</span> 단계가 결정됩니다.
          </div>
          {CLASSIFICATION_DEFS.map((g) => (
            <div key={g.groupTitle}>
              <div className="text-[12px] font-semibold text-gray-300 mb-2 flex items-baseline gap-2">
                <span className="text-base">{g.emoji}</span>
                <span>{g.groupTitle}</span>
                <span className="text-[10px] text-gray-500">({g.defs.length}개)</span>
              </div>
          ...
          <div className="bg-[#111827] border border-cyan-900/40 rounded p-2.5 text-[10.5px] text-cyan-200/80 leading-relaxed">
    ...
    <div className="border border-gray-800 rounded-lg overflow-hidden">
      ...
      >
        <span className="text-gray-400">
          ...
          <span className="ml-2 text-[10px] text-gray-600">pm_age · momentum_age · trend_age · days_in_stage</span>
        </span>
        <span className="text-gray-500 text-[10px]">{open ? "\u25bc" : "\u25b6"}</span>
      </button>
      {open && (
        <div className="bg-[#0d1117] p-4 space-y-4">
          <div className="text-[11px] text-gray-500 leading-relaxed">
            ...
            상태 종류에 따라 <span className="text-gray-300">4가지 age 개념</span>이 사용됩니다.
            가장 핵심은 <span className="text-cyan-300">pm_age</span> (Pre-Momentum 지속일)와
            <span className="text-green-300"> momentum_age</span> (Momentum 지속일)입니다.
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            ...
            {/* ── Pre-Momentum Age ── */}
            <div className="bg-[#111827] border-2 border-cyan-800/40 rounded-lg p-3">
              <div className="flex items-baseline justify-between mb-2">
                <div className="flex items-baseline gap-2">
                  <span className="text-cyan-300 text-base font-bold">🔵 pm_age</span>
                  <span className="text-[11px] text-gray-400">— Pre-Momentum 지속일</span>
                </div>
                <span className="text-[9.5px] px-1.5 py-0.5 rounded bg-cyan-900/30 text-cyan-300 border border-cyan-800/40 font-semibold">
                  Pre-Momentum stage
                </span>
              </div>

              <div className="text-[11px] text-gray-300 mb-2 leading-relaxed">
                <b className="text-gray-200">언제 적용:</b> Pre-Momentum 분류군에 속한 ticker (NEUTRAL · CONSOLIDATION · RECOVERY · PULLBACK · WEAKENING · FADING).
              </div>

              <div className="bg-[#0d1117] rounded p-2 mb-2 text-[10.5px] text-gray-400">
                <div className="text-cyan-300 text-[10px] font-semibold mb-1">📐 산출 알고리즘</div>
                <ol className="list-decimal pl-5 space-y-0.5">
                  <li>2주 간격(bi-weekly) 과거 observations를 새것부터 거꾸로 walk</li>
                  <li>분류가 PM 분류군이면 → 카운트 추가</li>
                  <li>분류가 momentum-confirmed (CONTINUATION/FORMATION 등)면 → <span className="text-yellow-300">중단 (breakout = age reset)</span></li>
              ...
              <div className="bg-[#0d1117] rounded p-2 mb-2 text-[10.5px] text-gray-400">
                <div className="text-cyan-300 text-[10px] font-semibold mb-1">🎯 의미</div>
                <ul className="list-disc pl-5 space-y-0.5">
                  <li><span className="text-green-300">짧음 (≤ 14일)</span>: 신선한 setup — 진입 적기 가능</li>
                  <li><span className="text-yellow-300">중간 (15~45일)</span>: 형성 중 — 모니터링</li>
                  <li><span className="text-red-300">길음 (&gt; 45일)</span>: 오래 정체 — pre-momentum 신뢰도 ↓</li>
                </ul>
              </div>

              <div className="text-[9.5px] text-gray-500 italic">
                구현: <code className="text-gray-300">_enrich_pm_age_robust()</code> in pre_momentum.py · 데이터: <code className="text-gray-300">.pm_history.json</code>
              </div>
            </div>

            {/* ── Momentum Age ── */}
            <div className="bg-[#111827] border-2 border-green-700/50 rounded-lg p-3">
              <div className="flex items-baseline justify-between mb-2">
                <div className="flex items-baseline gap-2">
                  <span className="text-green-300 text-base font-bold">🟢 momentum_age</span>
                  <span className="text-[11px] text-gray-400">— Momentum 지속일 (3-tier)</span>
                </div>
                <span className="text-[9.5px] px-1.5 py-0.5 rounded bg-green-900/30 text-green-300 border border-green-800/40 font-semibold">
                  Momentum stage
                </span>
              </div>

              <div className="text-[11px] text-gray-300 mb-2 leading-relaxed">
                <b className="text-gray-200">언제 적용:</b> Momentum 적격 ticker (composite ≥ 55 + ADV ≥ $5M + non-bearish).
              </div>

              <div className="bg-[#0d1117] rounded p-2 mb-2">
                <div className="text-green-300 text-[10px] font-semibold mb-1">📊 3-Tier Classification</div>
                <table className="w-full text-[10.5px]">
                  <tbody>
                    <tr className="border-b border-gray-800/50">
                      <td className="py-1 px-1 text-green-300 font-semibold">Tier A (확정)</td>
                      <td className="py-1 px-1 text-gray-300 font-mono text-[10px]">CONTINUATION · FORMATION · OVEREXTENDED</td>
                      <td className="py-1 px-1 text-right text-green-300 text-[10px]">→ 카운트</td>
                    </tr>
                    <tr className="border-b border-gray-800/50">
                      <td className="py-1 px-1 text-yellow-300 font-semibold">Tier B (조정)</td>
                      <td className="py-1 px-1 text-gray-300 font-mono text-[10px]">RECOVERY · CONSOLIDATION · PULLBACK</td>
                      <td className="py-1 px-1 text-right text-yellow-300 text-[10px]">→ gap (최대 2)</td>
                    </tr>
                    <tr>
                      <td className="py-1 px-1 text-red-300 font-semibold">Tier C (위험)</td>
                      <td className="py-1 px-1 text-gray-300 font-mono text-[10px]">DOWNTREND · CYCLE_PEAK · COUNTER_RALLY · EXHAUSTING · FADING · WEAKENING</td>
                      <td className="py-1 px-1 text-right text-red-300 text-[10px]">→ gap (최대 1)</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div className="bg-[#0d1117] rounded p-2 mb-2 text-[10.5px] text-gray-400">
                <div className="text-green-300 text-[10px] font-semibold mb-1">📐 산출 알고리즘</div>
                <ol className="list-decimal pl-5 space-y-0.5">
                  <li>새것부터 거꾸로 ve_observations walk</li>
                  <li>Tier A → 카운트 / Tier B → gap (최대 2 누적) / Tier C → gap (최대 1)</li>
                  <li>총 gap 누적 3 초과 시 즉시 중단</li>
                  <li><span className="text-red-300">Tier C 2연속</span> → hard break (실제 추세 전환)</li>
              ...
              <div className="bg-[#0d1117] rounded p-2 mb-2 text-[10.5px] text-gray-400">
                <div className="text-green-300 text-[10px] font-semibold mb-1">🎯 의미</div>
                <ul className="list-disc pl-5 space-y-0.5">
                  <li><span className="text-green-300">짧음 (≤ 21일)</span>: 신규 진입 — 추세 신선</li>
                  <li><span className="text-cyan-300">중간 (22~90일)</span>: 활성 추세 — 보유 유지</li>
                  <li><span className="text-yellow-300">길음 (&gt; 90일)</span>: 오래된 추세 — exhaust 위험 모니터링</li>
                </ul>
              </div>

              <div className="text-[9.5px] text-gray-500 italic">
                구현: <code className="text-gray-300">compute_momentum_ages()</code> in pre_momentum.py
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#111827] rounded-lg p-3">
            <div className="text-[12px] font-semibold text-gray-200 mb-2">📈 Gap Tolerance — 예시 (momentum_age)</div>
            <div className="text-[10.5px] text-gray-400 mb-2">
                ...
                <thead>
                  <tr className="border-b border-gray-700">
                    ...
                    {Array.from({length: 14}, (_, i) => (
                      <th key={i} className="px-1.5 py-1 text-center text-gray-500 w-[34px]">D-{13-i}</th>
                    ))}
                    <th className="px-2 py-1 text-left text-gray-300">결과</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b border-gray-800/50">
                    <td className="px-2 py-1 text-gray-400">분류</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-1.5 py-1 text-center text-yellow-400">B</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-1.5 py-1 text-center text-yellow-400">B</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-1.5 py-1 text-center text-red-400">C</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-2 py-1 text-green-300">age = 14일 ✓</td>
                  </tr>
                  <tr>
                    <td className="px-2 py-1 text-gray-400">분류</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    ...
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-1.5 py-1 text-center text-green-400">A</td>
                    <td className="px-2 py-1 text-red-300">break! Tier C 2연속 → age = 8일</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="bg-[#111827] rounded-lg p-3">
            <div className="text-[12px] font-semibold text-gray-200 mb-2">📎 보조 Age 개념 (참고)</div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-2.5">
              <div className="bg-[#0d1117] border border-gray-800 rounded p-2.5">
                <div className="font-mono text-[12px] font-bold text-orange-300 mb-1">trend_age</div>
                <div className="text-[10.5px] text-gray-300 mb-1">close &gt; SMA50인 연속 일수</div>
              ...
              <div className="bg-[#0d1117] border border-gray-800 rounded p-2.5">
                <div className="font-mono text-[12px] font-bold text-yellow-300 mb-1">trend_age_short</div>
                <div className="text-[10.5px] text-gray-300 mb-1">close &gt; SMA20인 연속 일수</div>
                <div className="text-[10px] text-gray-500 mb-1">
              ...
              <div className="bg-[#0d1117] border border-gray-800 rounded p-2.5">
                <div className="font-mono text-[12px] font-bold text-purple-300 mb-1">days_in_stage</div>
                <div className="text-[10.5px] text-gray-300 mb-1">동일 stage 유지 일수 (lifecycle)</div>
                <div className="text-[10px] text-gray-500 mb-1">
          ...
          <div className="bg-[#0a0e17] border border-cyan-900/40 rounded-lg p-3 text-[11px] text-cyan-200/90 leading-relaxed">
            <b className="text-cyan-100">📌 정리</b> —&nbsp;
            <span className="text-gray-300">
  ...
  for (let i = 0; i < clsLabels.length; i++) {
    for (let j = 0; j < data.dates.length; j++) {
      ...
      annotations.push({
        ...
        showarrow: false,
        font: { color: val > 80 ? "#000" : "#e5e7eb", size: 11, family: "monospace" },
      });
    }
  }
  ...
  return (
    <div className="border border-gray-800 rounded-lg overflow-hidden">
      <button
        ...
        onClick={() => setOpen(!open)}
      >
        <span className="text-gray-400">Classification Distribution — Bi-weekly Change ({data.dates.length} periods)</span>
        <span className="text-gray-500 text-[10px]">{open ? "\u25bc" : "\u25b6"}</span>
      </button>
      {open && (
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_140px] gap-4">
            ...
            <Plot
              data={[
                {
                  ...
                  showscale: true,
                  colorbar: { tickfont: { color: "#9ca3af", size: 9 }, len: 0.8, thickness: 12 },
              ...
              layout={{
                ...
                margin: { t: 10, b: 60, l: 160, r: 60 },
                xaxis: { tickangle: -30, tickfont: { size: 10, color: "#9ca3af" }, side: "bottom" },
            ...
            <div className="space-y-0 pt-2">
              <div className="text-[9px] text-gray-500 uppercase tracking-wide mb-1 text-center">Latest Change</div>
              {clsLabels.map((cls, i) => {
                const d = deltas[i];
                const color = d > 10 ? C.green : d > 0 ? "#86efac" : d < -10 ? C.red : d < 0 ? "#fca5a5" : C.gray;
                return (
                  <div key={cls} className="flex items-center justify-between px-2 py-[3px] text-[10px]" style={{ height: "32px" }}>
                    <span className="text-gray-500 truncate max-w-[80px]">{cls.replace(/^.\s/, "")}</span>
  ...
  return (
    <div className="bg-[#111827] border border-gray-800 rounded-lg p-3">
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-semibold text-gray-200">{entry.sector.replace("STK_", "").replace("EQ_", "")}</span>
        <span className="text-[10px] text-gray-500">{entry.total} tickers</span>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-[10px] border-collapse">
          <thead>
            <tr className="border-b border-gray-800">
              <th className="py-1 px-1 text-left text-gray-600 w-12">Class</th>
              {dates.map((d) => (
                <th key={d} className="py-1 px-1 text-center text-gray-600 w-8">{d.slice(5)}</th>
              ))}
              <th className="py-1 px-1 text-center text-gray-600 w-8">\u0394</th>
            </tr>
          </thead>
          <tbody>
            {entry.classifications.map((cls, ci) => {
              ...
              return (
                <tr key={cls} className="border-b border-gray-800/30">
                  ...
                  {row.map((v, di) => (
                    <td key={di} className="py-0.5 px-1 text-center font-mono"
                        style={{ color: v > 0 ? "#e5e7eb" : "#374151" }}>
                      {v > 0 ? v : "\u00b7"}
                    </td>
                  ))}
                  <td className="py-0.5 px-1 text-center font-mono font-bold"
                      style={{ color: delta > 0 ? "#22c55e" : delta < 0 ? "#ef4444" : "#6b7280" }}>
...
function SectorClassificationGrid({ data }: { data: SectorEntry[] | null }) {
  ...
  return (
    <div className="border border-gray-800 rounded-lg overflow-hidden">
      <button
        ...
        onClick={() => setOpen(!open)}
      >
        <span className="text-gray-400">Classification by Sector ({data.length} sectors)</span>
        <span className="text-gray-500 text-[10px]">{open ? "\u25bc" : "\u25b6"}</span>
      </button>
      {open && (
        <div className="bg-[#0d1117] p-4">
    ...
    <div className="bg-[#111827] border border-gray-800 rounded-lg p-4 space-y-3">
      ...
      <div className="relative">
        <input
          ...
          className="w-full px-3 py-2 bg-[#0d1117] border border-gray-700 rounded text-sm text-gray-200 placeholder-gray-600 focus:border-cyan-600 focus:outline-none"
        />
        {query && !selected && results.length > 0 && (
          <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-[#1f2937] border border-gray-700 rounded shadow-lg max-h-60 overflow-y-auto">
            {results.map((r) => {
              ...
              return (
                <button
                  ...
                  className="w-full px-3 py-2 text-left hover:bg-[#374151] flex items-center gap-3 text-xs border-b border-gray-800/50 last:border-0"
                >
                  <span className="font-mono font-bold text-cyan-400 w-16">{r.ticker}</span>
                  ...
                  <span className="font-mono text-gray-500 w-12 text-right">{r.composite?.toFixed(1)}</span>
                </button>
              );
            })}
          </div>
        ...
        {query && !selected && results.length === 0 && (
          <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-[#1f2937] border border-gray-700 rounded px-3 py-2 text-xs text-gray-500">
            No results
          </div>
        )}
      </div>
      ...
      {selected && badge && (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <span className="text-lg font-bold text-cyan-400 font-mono">{selected.ticker}</span>
            ...
            <button
              ...
              className="text-[10px] text-cyan-500 hover:text-cyan-300 underline ml-auto"
            >
          ...
          <div className="grid grid-cols-4 md:grid-cols-8 gap-2">
            {[
              ...
              <div key={a.label} className="bg-[#0d1117] rounded p-2 text-center">
                <div className="text-[9px] text-gray-500">{a.label}</div>
                <div className="text-sm font-bold font-mono" style={{ color: a.color }}>
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
            ...
            <div className="bg-[#0d1117] rounded p-2">
              <div className="text-[9px] text-gray-500">L / S</div>
              <div className="text-xs font-mono">
                <span className="text-green-400">{selected.long_count ?? 0}</span>
                <span className="text-gray-600"> / </span>
                <span className="text-red-400">{selected.short_count ?? 0}</span>
              </div>
            </div>
            <div className="bg-[#0d1117] rounded p-2">
              <div className="text-[9px] text-gray-500">ADV</div>
              <div className="text-xs font-mono">${selected.adv_M?.toFixed(1)}M</div>
            </div>
            <div className="bg-[#0d1117] rounded p-2">
              <div className="text-[9px] text-gray-500">1W Return</div>
              <div className="text-xs font-mono" style={{ color: retColor(selected.ret_1w) }}>
                {selected.ret_1w?.toFixed(1)}%
              </div>
            </div>
            <div className="bg-[#0d1117] rounded p-2">
              <div className="text-[9px] text-gray-500">1M Return</div>
              <div className="text-xs font-mono" style={{ color: retColor(selected.ret_1m) }}>
                {selected.ret_1m?.toFixed(1)}%
              </div>
            </div>
            <div className="bg-[#0d1117] rounded p-2">
              <div className="text-[9px] text-gray-500">3M Return</div>
              <div className="text-xs font-mono" style={{ color: retColor(selected.ret_3m) }}>
          ...
          {selected.stage === "pre-momentum" && selected.pm_score != null && (
            <div className="border border-cyan-900/40 rounded-lg p-3 space-y-2">
              <div className="text-[10px] text-cyan-400 font-semibold uppercase tracking-wide">Pre-Momentum Signals</div>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                <div className="bg-[#0d1117] rounded p-2 text-center">
                  <div className="text-[9px] text-gray-500">PM Score</div>
                  <div className="text-sm font-bold font-mono text-cyan-400">{selected.pm_score.toFixed(1)}</div>
                </div>
                <div className="bg-[#0d1117] rounded p-2 text-center">
                  <div className="text-[9px] text-gray-500">Agreement</div>
                ...
                <div className="bg-[#0d1117] rounded p-2 text-center">
                  ...
                  <div className="text-xs text-gray-300">{selected.pm_timeline}</div>
                </div>
                {selected.pm_agents && (
                  <>
                    <div className="bg-[#0d1117] rounded p-2 text-center">
                      <div className="text-[9px] text-gray-500">Micro / Macro</div>
                      <div className="text-xs font-mono text-gray-300">
                        {selected.pm_agents.microstructure?.score?.toFixed(0)} / {selected.pm_agents.macro_regime?.score?.toFixed(0)}
                      </div>
                    </div>
                    <div className="bg-[#0d1117] rounded p-2 text-center">
                      <div className="text-[9px] text-gray-500">Graph / Catalyst</div>
                      <div className="text-xs font-mono text-gray-300">
              ...
              {selected.pm_catalysts && selected.pm_catalysts.length > 0 && (
                <div className="text-[11px] text-gray-400">
                  <span className="text-gray-500">Catalysts: </span>
...
export function PriceDiscoveryTab({ filters }: { filters: FilterParams }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <SectorClassificationGrid data={sectorHistory} />
      ...
      {sub === 2 && <ExcludedTab filters={filters} totalUniverse={allResults.length} />}
    </div>
  );
}
